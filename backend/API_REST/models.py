...
class Course(models.Model):
	...
	def __unicode__(self):
		return u'%s [%d]' % (self.name, self.courseName)
...
class UserManager(BaseUserManager):
	...
	def _create_user(self, fullName, email, cpf, rg, registration, phone1, phone2, password, is_staff, is_superuser, **extra_fields):
		now = timezone.now()
		if not registration:
			#raise ValueError(_(‘The given username must be set’))
			print("erro")
		email = self.normalize_email(email)
		user = self.model(fullName=fullName, email=email,cpf=cpf, rg=rg, registration=registration, phone1=phone1, phone2=phone2, is_staff=is_staff, is_active=True, is_superuser=is_superuser, last_login=now, date_joined=now, **extra_fields)
		user.set_password(password)
		user.save(using=self._db)
		return user
	
	def create_user(self, fullName, email, phone1, phone2, rg, cpf, password, is_staff, is_superuser, **extra_fields):
		print(fullName)
		return self._create_user(fullName, email, phone1, phone1, rg, cpf, password, False, False, **extra_fields)
	
	def create_superuser(self, fullName, email, cpf, rg, registration, phone1, phone2, password, **extra_fields):

		user=self._create_user(fullName, email, cpf, rg, registration, phone1, phone2, password, True, True, **extra_fields)
		user.is_active=True
		user.save(using=self._db)
		return user
...
class Student(AbstractBaseUser, PermissionsMixin):
	...
	id_course = models.PrimaryKeyRelatedField(Course, related_name='course' ,on_delete=models.CASCADE)




	USERNAME_FIELD = 'registration'
	REQUIRED_FIELDS = ['fullName', 'email', 'cpf', 'rg', 'phone1', 'phone2']
